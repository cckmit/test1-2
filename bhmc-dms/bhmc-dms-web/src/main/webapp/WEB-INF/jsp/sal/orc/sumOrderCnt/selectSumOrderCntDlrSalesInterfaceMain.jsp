...
<div id="resizableContainer">
    <section class="group">
        <div class="header_area">
            <h1 class="title_basic"><spring:message code='global.title.sumOrderCntDlr' /></h1><!-- 종합오더현황-딜러 VIEW-D-10969 -->
            <div class="btn_right">
                <dms:access viewId="VIEW-D-13303" hasPermission="${dms:getPermissionMask('READ')}">
...
<%--             <div class="btn_left">
            ...
            <div class="header_area">
                <div class="btn_right"><!-- VIEW-D-13277 -->
                    <dms:access viewId="VIEW-D-13304" hasPermission="${dms:getPermissionMask('READ')}">
...
$(document).ready(function() {
    ...
    $("#btnExcelExport1").kendoButton({
       click:function(e){
           ...
           var dtOrdDtSF = kendo.toString(sOrdStartDt, 'yyyy-MM-dd');//下单日期 开始
           var dtOrdDtEF = kendo.toString(sOrdEndDt, 'yyyy-MM-dd');//下单日期 结束
           ...
           */
           ...
           var sMainSubDivision = false;  // true 일때가 메인 그리드임 ..
           ...
           if(srchTp == 'Y'){ // 정규오더
               dms.ajax.excelExport({
                   ...
                   ,"sIdentification":"orderSalesInter"
                   ,"firstIndex":0
                   ,"lastIndex":65530
               });
           }else{ // 비정규 오더
                 ...
                 //,"sOrdStartDtstrTmp":sOrdStartDtstrTmp
                   //,"sOrdEndDtstrTmp":sOrdEndDtstrTmp 贾明
               dms.ajax.excelExport({
                   "beanName":"sumOrderCntService"
                   ,"templateFile":"sumOderCntDlrMainInfo_Tpl.xlsx"
                   ,"fileName":"sumOderCntDlrMainInfo.xlsx"
                   ,"strOrdTp":strOrdTp // 주문 유형
                   ,"sOrdsTp":sOrdsTp //
                   ,"sOrdStartFormatDt":dtOrdDtSF
                   ,"sOrdEndFormatDt" :dtOrdDtEF
                   ,"sCarlineCd":$("#sCarlineCd").data("kendoExtDropDownList").value() // 차종
                   // 여기 까지가 기본
                    ,"sModelCd":sModelCd //
                   ,"sFscCd":sFscCd //
   ...
   // 엑셀 상단부 export 버튼
         ...
         var sMainSubDivision = false; // false 일때는 서브 그리드 ..
           dms.ajax.excelExport({
               ...
               ,"fileName":"sumOderCntDlrSubInfo.xlsx"
                ,"sDlrCd":selectedItem.dlrCd == null ? "" : selectedItem.dlrCd
                 , "sOrdYyMmDt":selectedItem.ordYyMmDt == null ? "":selectedItem.ordYyMmDt
                 , "sOrdPrid":selectedItem.week == null ? "": selectedItem.week
                 , "sOrdsTp":selectedItem.ordTp == null ? "":selectedItem.ordTp
                 , "sCarlineCd":selectedItem.carlineCd == null ? "":selectedItem.carlineCd
                 , "sModelCd":selectedItem.modelCd == null ? "":selectedItem.modelCd
                 , "sOcnCd":selectedItem.ocnCd == null ? "":selectedItem.ocnCd
                 , "sExtColorCd":selectedItem.extColorCd == null ? "":selectedItem.extColorCd
                 , "sIntColorCd":selectedItem.intColorCd == null ? "":selectedItem.intColorCd
                 , "sOrdNo":selectedItem.ordNo == null ? "":selectedItem.ordNo
                 //2018-12-28 add byfengdequan 明细显示根据下单人ID
                 , "sregUsrId":selectedItem.regUsrId == null ? "": selectedItem.regUsrId
                 , "sordGradeCd":selectedItem.ordGradeCd == null ?"":selectedItem.ordGradeCd
                ,"sMainSubDivision" : sMainSubDivision
                ,"sComOderDetai" : "comOderDetai" 
           });
    ...
    // 메인 그리드  selectSumOrderCntDlrs
    $("#mainGrid").kendoExtGrid({
        gridId:"G-SAL-0322-114112"
        ,dataSource:{
            transport:{
                read:{
                    url:"<c:url value='/sal/orc/sumOrderCnt/selectSumOrderCntSalesInterfaceDlrs.do' />"
                },
                parameterMap:function(options, operation) {
                    if (operation === "read") {
                        ...
                        var strOrdTp = $("#sOrdTp").data('kendoExtDropDownList').value();
                        var strOrdYyMmPrid = $("#sOrdYyMmPrid").data('kendoExtDropDownList').value();
                        var dtOrdDtS = $("#sOrdDtS").data('kendoDatePicker').value();//下单日期 开始
                        var dtOrdDtE = $("#sOrdDtE").data('kendoDatePicker').value();//下单日期 结束
                        
                      
                        
                        
                       var dtOrdDtSF = kendo.toString(dtOrdDtS, 'yyyy-MM-dd');//下单日期 开始
                       var dtOrdDtEF = kendo.toString(dtOrdDtE, 'yyyy-MM-dd');//下单日期 结束
                       //console.log("dtOrdDtSF-->",dtOrdDtSF);
                       //console.log("dtOrdDtEF-->",dtOrdDtEF);
                        var sModelCd = "";
                        var sFscCd = "";
                        if(sOrdTp == "Y"){
                            dtOrdDtS = "";
                        ...
                        params["sOrdStartFormatDt"] = dtOrdDtSF;
                        params["sOrdEndFormatDt"] = dtOrdDtEF;
...
//click 
$("#mainGrid").on("click", "tr.k-state-selected", function (e) {
        //console.log("单击"); 
    	var grid = $("#mainGrid").data("kendoExtGrid");
       var selectedItem = grid.dataItem(grid.select());
        //console.log("执行2");
   ...
   $("#subGrid").kendoExtGrid({
       ...
       ,dataSource:{
           transport:{
               read:{
                   url:"<c:url value='/sal/orc/sumOrderCnt/selectSumOrderCntSalesInterfaceDlrSubs.do' />"
               },
               parameterMap:function(options, operation) {
                   if (operation === "read") {
                       //var params = {};
